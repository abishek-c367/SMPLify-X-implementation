...
# The variable degrees contains a list of camera/body rotation angles.
# If the user did not provide any value (degrees=None), the code uses [0 , 90 , 180 , 270]
    if degrees is None:
...
#===============================================================================>>>>>
    ...
    #Initialize placeholders so later code can pass them (or None) into the fitting.create_loss(...) call.
    search_tree = None          
    pen_distance = None
    filter_faces = None
    if interpenetration: #activated when the user requests collision penalties:
        try:
            from mesh_intersection.bvh_search_tree import BVH  #Imports the BVH implementation (accelerates collision queries).
            import mesh_intersection.loss as collisions_loss   #Imports the differentiable collision/penetration loss utilities.
            from mesh_intersection.filter_faces import FilterFaces #Imports a helper that filters/ignores certain face-pairs (e.g., adjacent parts).
        except ModuleNotFoundError:
            interpenetration = False
            coll_loss_weights = None
            print(
                'Warning: interpenetration is enabled in configuration, but ' \
                'the optional package "mesh_intersection" is not installed. ' \
                'Collision penalties will be disabled automatically.')
        else:
            ...
            search_tree = BVH(max_collisions=max_collisions) #Instantiate the BVH with a limit on returned collisions to bound compute.

            pen_distance = \
                collisions_loss.DistanceFieldPenetrationLoss( #Create the loss module that computes penetration distances
...
#===========================================
    ...
    with fitting.FittingMonitor(
        ...
        # orientation 
        if try_both_orient:
            body_orient = body_model.global_orient.detach().cpu().numpy() #OpenCV only accepts CPU numpy arrays.
            flipped_orient = cv2.Rodrigues(body_orient)[0].dot(
                cv2.Rodrigues(np.array([0., np.pi, 0]))[0])           #Create a 180° Y-axis rotation

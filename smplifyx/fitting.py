...
class FittingMonitor(object):
    ...
    def run_fitting(self, optimizer, closure, params, body_model,
        ...
        ''' Helper function for running an optimization process
            ...
            The key idea is that run_fitting() is optimizer-agnostic. It doesn't know the mathematical form of the objective; all of that is encapsulated in the closure. 
            Its responsibilities are to drive the optimization loop, monitor convergence using both function tolerance (ftol) and gradient tolerance (gtol), guard against
            numerical failures (NaN/Inf), and optionally visualize the evolving SMPL mesh. This separation makes the same optimization loop reusable for different fitting 
            stages, such as camera initialization, body pose optimization, and shape optimization, simply by providing a different closure and a different set of parameters.
        '''

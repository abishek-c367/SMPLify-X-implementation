...
def exprdir2model(expr_dir, model_cfg_override: dict = None):
    # expr_dir --> C:\Abishek\SMPLify\human_body_prior\support_data\dowloads\V02_05
    ...
    trained_weights_fname = available_ckpts[-1]  # path to the last checkpoint file of VPoser
    print(trained_weights_fname)

    # print(trained_weights_fname.split('\\')[:])
    model_cfg_fname = glob.glob(osp.join('\\', '\\'.join(trained_weights_fname.split('\\')[:-2]), '*.yaml'))#.as_posix()
    print(model_cfg_fname)
    ...
    if model_cfg_override:
        ...
        # trained_weights_fname is the pretrained weights of VPoser that we have downloaded 
...
def load_model(expr_dir, model_code=None,
    ...
    #loads weights in the architecture 
    ...
    #model_instance is the VPoser architecture with loaded weights and model_cfg is the configuration file of the Vposer architecture

def load_vposer(expr_dir, vp_model='snapshot'):
    from human_body_prior.human_body_prior.models.vposer_model import VPoser
    # from human_body_prior.tools.model_loader import load_model
    vposer_pt, model_cfg = load_model(
        expr_dir,
        model_code=VPoser,
        remove_words_in_model_weights='vp_model.',
        disable_grad=True
    )
    return vposer_pt, model_cfg
